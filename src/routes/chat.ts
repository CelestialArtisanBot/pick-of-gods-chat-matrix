...
export async function handleChat(req: Request, env: Env): Promise<Response> {
  ...
  if (openRouterKey) {
    ...
    let log: any = { messages: body.messages, intent, safe: isSafe, generateImage: body.generateImage, timestamp: generateTimestamp() };
    ...
    if (env.DISPATCHER && typeof env.DISPATCHER.fetch === 'function') {
      response = await dispatchRequest(intent, body.messages, body.generateImage, env);
    } else {
      switch (intent) {
        case 'image_generation':
          response = await callTextToImageTemplate(body.messages[body.messages.length - 1].content, body.generateImage, env);
          break;
        case 'chat':
...
async function dispatchRequest(intent: string, messages: Message[], generateImage: boolean, env: Env): Promise<any> {
  try {
    const response = await env.DISPATCHER.fetch('https://dispatcher.celestialartisanbot.workers.dev/api', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ intent, messages, generateImage }),
    });
...
async function callTextToImageTemplate(prompt: string, generateImage: boolean, env: Env): Promise<any> {
  if (!generateImage) return { success: true, message: 'Image generation disabled' };
